...
3. **Required, easy to miss**: the `at-trunk` endpoint is registered with Africa's Talking as a *SIP phone*, not a true SIP trunk — AT answers that leg onto its own platform and independently fires its account-wide Voice callback URL (`/voice` in `at-voice-app/app.js`) asking what to do with it, completely outside Asterisk's own signaling. `/voice` must respond with `<Dial phoneNumbers="{callerNumber}" callerId="{destinationNumber}"/>` (both values echoed straight from AT's own webhook payload) or AT never actually connects the call — an empty response, a `<Hangup/>`, or a 404 all fail identically (confirmed live 2026-08-19: `180 Ringing` then an immediate `480` on the SIP side, a 0-duration "Aborted" session in AT's dashboard). This has nothing to do with inbound calls, which AT delivers over plain SIP with no callback dance.
4. The destination channel can reach `'Up'` either immediately (a fast pickup) or after genuinely ringing for a while — both are handled, since a channel already `Up` the moment it's first seen will never fire an *additional* state-change event. Once `Up`, the two legs are bridged: `status: 'ongoing'`, agent `status: 'on_call'` (without this, an agent looked "available" for ring-all purposes for the entire duration of a call they were already on).
5. A destination that never answers (busy, rejected, times out) ends the call `failed`; a bridged call that completes normally ends `completed` with duration.
...
**`/voice` itself had to stay, though — it is not dead**, despite looking like it belonged to the same removal. See "Outbound (agent-initiated)" above for the full explanation: `at-trunk` is actually an AT-hosted *SIP phone*, not a true SIP trunk, and AT requires a `<Dial>` response from `/voice` to complete every outbound call. Three things were tried and confirmed live, via AT's own dashboard, to fail identically (0-duration "Aborted" session, `480` on the SIP side): the original `<Hangup/>` catch-all, removing the route entirely (AT's webhook got a 404 instead), and an empty `<Response/>`. Only echoing back `<Dial phoneNumbers="{callerNumber}" callerId="{destinationNumber}"/>` — using AT's own payload values — actually works.

`/events` also remains — it's Africa's Talking's separate account-wide call-events callback, unrelated to the Voice URL — but whether it still receives any real traffic now that calls route through the SIP trunk is unverified; treat its writes to `call_logs` with the same caution as before.
