...
**`/voice` itself had to stay, though — it is not dead**, despite looking like it belonged to the same removal. Confirmed live: Africa's Talking's account-wide Voice URL fires for *any* call presenting an AT-owned caller ID at all, including every outbound call placed through the self-hosted Asterisk SIP trunk — completely unrelated to the phone-standby fallback — and it *requires* a valid XML response or it aborts the call outright. The original `<Hangup/>` catch-all ("nothing else should reach this URL") was killing every SIP-trunk outbound call the instant it started ringing; removing the route entirely made things worse, not better — AT's webhook then got a 404, which killed the call exactly as dead (both confirmed in AT's own dashboard: a 0-duration "Aborted" session, a `480` on the SIP side seconds after `180 Ringing`). `/voice` now unconditionally responds with an empty `<Response></Response>` and does nothing else — there's nothing left for it to control, since the SIP trunk already fully owns the call.
